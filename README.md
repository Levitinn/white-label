# Начало
- Стягиваем проект к себе на машину
- В корне проекта пишем ```npm install```
- После того, как подтянулись все зависимости, можем запустить проект
```npm run start```
- Ждем компиляции всех файлов, после чего проект автоматически открывается в браузере
- Для правки файлов сохраняем редактируемый файл и страница автоматически перезагружается
# После добавления в проект новых изображений:
   Необходимо перезапустить сборку (ctrl + C), после чего снова запустить ```npm run start``` 
